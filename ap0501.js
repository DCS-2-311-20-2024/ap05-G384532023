...
function init() {
  ...
  // 自動販売機の作成
  // 素材に関する処理
  const vender01Texture = textureLoader.load("vender01.jpg");
  vender01Texture.colorSpace = THREE.SRGBColorSpace;
  const vender02Texture = textureLoader.load("vender02.jpg");
  vender02Texture.colorSpace = THREE.SRGBColorSpace;
  const venderMaterial = new THREE.MeshPhongMaterial({ color: 0xE8E7E5 });
  const vender01Material = new THREE.MeshPhongMaterial({ 
    map: vender01Texture
  });
  const vender02Material = new THREE.MeshPhongMaterial({ 
    map: vender02Texture
  });
  ...
  // 自販機1の位置の設定
  vender01.position.set(-0.6, 0.9, 0);
  ...
  // 自販機2の位置の設定
  vender02.position.set(0.5, 0.9, 0);
